...
**Revocation not implemented.** The verifier emits a `revocation check` step with a fixed "not implemented — no revocation list defined yet" message in all four implementations. The revocation protocol is fully specified in SPEC.md §Revocation — a signed Bloom filter cascade distributed at `GET /revoked` on the charge-cycle schedule. Key deployment notes: revocation propagation latency is bounded by the verifier's charge cycle (typically hours); Mode 0 deployments cannot perform online revocation checks and must pre-load an artifact or rely on TTL expiry; Mode 2 can fetch the artifact at scan time for near-real-time revocation. Implementing the cascade construction is the primary remaining SDK task before production use.
