...
**Revocation not implemented.** The verifier emits a `revocation check` step with a fixed "not implemented — no revocation list defined yet" message in all four implementations. The revocation protocol is fully specified in SPEC.md §Revocation — a signed Bloom filter cascade distributed at `GET /revoked`, inspired by but not wire-compatible with CRLite. Revocation freshness depends on mode: Mode 1 is bounded by the charge cycle; Mode 2 can fetch a fresh artifact at each scan. Mode 0 verifiers can only check revocation against a pre-loaded cached artifact. Implementation is the primary remaining SDK task.
