/**
 * MTA-QR payload binary encoding and decoding.
 *
 * Big-endian integers throughout. All length fields are bounds-checked
 * before reading. Must produce identical byte sequences to the Go
 * implementation.
 */
...
export interface Payload {
   ...
   * innerProofCount is the split point between the two.
   */
...
export function encodePayload(p: Payload): Uint8Array {
  ...
  const pushU8  = (v: number) => push(new Uint8Array([v]));
  ...
  pushU8(p.proofHashes.length);
  pushU8(p.innerProofCount);
...
export function decodePayload(data: Uint8Array): Payload {
  ...
  const innerProofCount = r.u8("inner_proof_count");
  ...
  for (let i = 0; i < numProof; i++) proofHashes.push(r.bytes(32, `proof[${i}]`));
  ...
  if (mode === MODE_EMBEDDED) {
    ...
    const cosigCount = r.u8("witness_count");
    p.cosigs = [];
...
class Reader {
  ...
  bytes(n: number, field: string): Uint8Array {
    if (this.pos + n > this.data.length) {
      throw new Error(
        `payload: need ${n} bytes for ${field} at offset ${this.pos}, ` +
        `only ${this.data.length - this.pos} remaining`
      );
    }
