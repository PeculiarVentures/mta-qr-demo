...
`OriginID` is `SHA-256(origin)[0:8]` stored as a big-endian uint64. It is a routing hint, not a collision-resistant identifier. Checkpoint cache keys and revocation artifact cache keys MUST use the full origin string. The revocation artifact cache is keyed on `full_origin` only (not `full_origin + tree_size`) because the artifact is superseded entirely by each new publication, unlike checkpoints which are cached by `(full_origin, tree_size)` because old roots remain valid for old payloads.
